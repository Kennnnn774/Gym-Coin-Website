...
contract YourContract is Ownable {
  ...
  function addOneDevice(uint newDeviceID) public {
        User storage sender = Users[msg.sender];
  ...
  function addExercise(bytes32 userID, uint deviceID, uint heart_rate, uint workout_time_in_minutes, uint calories) public {
        User storage sender = Users[msg.sender];
        require(sender.registered == true, "You have not registered.");
  ...
  Post[] public posts; // post list

  function addPost(string memory context) public {
...
//     }
  ...
  function SignContract(uint contractID_100_or_200_or_300) public {
      User storage sender = Users[msg.sender];
      require(sender.registered == true, "You have not registered.");
      require(sender.contractID == 0, "You've already signed a contract. Please cancel it before you sign a new one.");

      sender.contractID = contractID_100_or_200_or_300;
